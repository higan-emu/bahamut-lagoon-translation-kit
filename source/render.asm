...
function bpp2 {
  ...
  renderCharacter: {
    ...
    cmp.w #command.base;        jcs renderCharacter
...
function width {
  ...
  loop: {
    ...
    cmp.w #command.alignSkip; bne +; {
      lda $0000,x; and #$00ff; inx
      add pixel; sta pixel; bra loop
    };+
    cmp.w #command.pause; bne +; inx; bra loop; +
    cmp.w #command.wait;  jcs epilogue
    cmp.w #command.base;  bcs loop
...
function bpp2 {
  ...
  namespace align {
    ...
    function center {
      lda index;  add.w #text >>  0; sta width.address+0
      lda #$0000; adc.w #text >> 16; sta width.address+2
      jsl width; cmp pixels; bcc +; beq +; lda.w #0; sta pixel; rtl; +
      pha; lda pixels; inc; sub $01,s; lsr; sta pixel; pla; rtl
    }
