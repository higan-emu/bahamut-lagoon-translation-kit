...
namespace tilemapTransfer {
  ...
  //A => 8-bit tile
  function hook {
    cmp #$fe; bcc +; jml control; +
    cmp #$01; bne +; lda $10; and #$fc; ora #$01; sta $10; jml read; +  //tile bank 1
    cmp #$02; bne +; lda $10; and #$fc; ora #$02; sta $10; jml read; +  //tile bank 2
    cmp #$03; bne +; lda $10; and #$e3; ora #$00; sta $10; jml read; +  //palette 0
    cmp #$04; bne +; lda $10; and #$e3; ora #$04; sta $10; jml read; +  //palette 1
    cmp #$05; bne +; lda $10; and #$e3; ora #$08; sta $10; jml read; +  //palette 2
    jml write
  }
}
...
namespace statusIcons {
  enqueue pc
  seek($c0e00c); ldx #$0628; jsl main; jmp $e01c
  seek($c0dcc6); ldx #$0629; jsl main; rts
  dequeue pc
  ...
  function main {
    ...
    enter; sep #$20; phx; ldx $18
    lda $7e0001,x; and.b #status.ailments.mask; sta ailments
    lda $7e0003,x; and.b #status.enchants.mask; sta enchants; plx
    lda ailments; and.b #status.ailments.bunny;     beq +; lda #$ec; sta $00,x; inx; +
    lda ailments; and.b #status.ailments.sleeping;  beq +; lda #$ee; sta $00,x; inx; +
    lda ailments; and.b #status.ailments.poisoned;  beq +; lda #$f0; sta $00,x; inx; +
    lda ailments; and.b #status.ailments.petrified; beq +; lda #$f1; sta $00,x; inx; +
    lda ailments; and.b #status.ailments.defeated;  beq +; lda #$f2; sta $00,x; inx; +
    lda enchants; and.b #status.enchants.bingo;     beq +; lda #$f5; sta $00,x; inx; +

    //fill remaining tilemap entries with spaces
    lda #$ef
  -;cpx #$0631; beq +
    sta $00,x; inx; bra -; +

    leave; ldx $18
    lda ailments; ora enchants  //Z <= !iconsDrawn; needed to draw "Boss" enemy string
    rtl
  }
}
...
namespace playerClass {
  ...
  function main {
    ...
    //if the player has any status effects, skip printing the class name
    lda $0728; and #$00ff; cmp #$00ef; beq +; leave; rtl; +
...
namespace dragonCommand {
  ...
  function main {
    ldx #$063f
    enter; and #$00ff
    cmp.w #3; bcc +; lda #$ffff; sta $0735; leave; rtl; +  //should never occur
...
namespace techniqueSmall {
  ...
  function main {
    ...
    lda #$0005; sta palette           //default to hinted palette
    ldx $1e; lda $0011,x; and #$00ff  //check if the text will be grayed out
    cmp #$00fe; bne +                 //(eg if there is insufficient MP/SP)
    lda #$0004; sta palette; +        //if so, use default palette instead

    //write the technique name
    lda.b name; and #$00ff; mul(8); tay
    lda #$0008; index.bpp2(); write.bpp2(lists.techniques.bpp2)
    txa; ldx #$0000
    ...
    //write the technique cost, unless it is $00
  +;lda.b cost; and #$00ff; jeq +
    ...
    mp:; plx; pla; write.bpp2(lists.costsMP.bpp2); bra wr
    sp:; plx; pla; write.bpp2(lists.costsSP.bpp2); wr:
    txa; plx

    sep #$20;    pha; lda palette  //write palette modifying code + three tiles
    sta $0720,x; inx; pla; xba
    sta $0720,x; inx; xba
    sta $0720,x; inx; inc
    sta $0720,x; inx; inc
    sta $0720,x; inx; rep #$20
...
namespace itemDrop {
  ...
  function piro {
    ...
    enter
    lda.w value
    ldx #$0000; append.alignSkip(6); append.integer_5(); append.literal(" Piro")
