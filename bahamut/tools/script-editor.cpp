...
auto ScriptEditor::load() -> void {
  ...
  for(auto& script : scripts) {
    ...
    for(u32 index : range(script.count)) {
      ...
      for(auto& text : english) {
        ...
        if(address == offset) script.english(index) = slice(text, 7).transform("{}", "<>");
      }
      for(auto& text : notes) {
        u16 address = slice(text, 1, 4).hex();
  ...
  englishTextRenderer.load("font-large", 8, 12);
}
...
auto ScriptEditor::englishDraw() -> void {
  ...
  if(script) {
    ...
    text.transform("<>", "{}");
    englishTextRenderer.drawScript(
      {canvas.data(), canvas.size()},
      {canvas.width(), canvas.height()},
